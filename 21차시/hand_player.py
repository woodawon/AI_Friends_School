...
hands = mp_hands.Hands(
    max_num_hands=1
)  # 손 인식 객체 생성, Hands()에 선언한 max_num_hands를 1로 설정하여 1개의 손만 인식한다.

end = video.get(
    cv2.CAP_PROP_FRAME_COUNT
)  # 동영상(video : boat.mp4) 파일의 전체 frame 수를 가져옵니다.
w = cap.get(
    cv2.CAP_PROP_FRAME_WIDTH
)  # 카메라로 촬영하는 영상(cap(비디오 캡처 객체) : camera)의 가로 길이를 가져옵니다.
...
(
    ...
    video.read()
)  # 동영상 데이터 읽기 -> 밑의 if else 조건에서 영상 실행 중 프레임 고려 때가 아니더라도 기본적으로 동영상 출력은 시켜줄 수 있어야 하니까 ..
while True:  # 무한 반복
    ...
    if results.multi_hand_landmarks:  # 손이 인식되었는지 확인
        hand_landmarks = results.multi_hand_landmarks[
            0
        ]  # 1개의 손만 인식하므로 결과를 hand_landmarks 변수에 저장
        ...
        if fingers == [0, 0, 0, 0, 0]:  # 손가락이 모두 접힌 경우 -> 주먹 일 때
            hand_shape = "rock"
        elif distance(points[4], points[8]) < 0.1 and fingers[2:] == [
            1,
            1,
            1,
        ]:  # 엄지 손가락과 검지 손가락이 닿아있고, 나머지 손가락 3개가 펴진 경우 -> ok일 때
            pos = int(
                # w => 비디오 캡처 객체의 가로 길이
                # end => 동영상파일의 전체 frame 수
                np.interp(
                    points[8].x * w, (50, w - 50), (1, end - 1)
                )  # np.interp(x, xp, yp)
                # xp - 데이터 포인트의 x좌표
                # yp - 데이터 포인트의 y좌표
            )  # 캠 화면의 손가락의 위치에 따라 동영상의 프레임 위치를 구한다.
            video.set(cv2.CAP_PROP_POS_FRAMES, pos)  # 프레임을 이동한다. ((pos 유의!))
            hand_shape = "OK"
    ...
    if (
        hand_shape != "rock"
        and video.get(cv2.CAP_PROP_POS_FRAMES)
        != end  # 비디오 캡처 객체의 pos 길이와 동영상 객체의 전체 frame 수가 같다면? -> 영상 종료된 것 의미함.
    ):  # 동영상이 끝나지 않았을 경우
        video_res, video_frame = video.read()  # 동영상 데이터 읽기
        if not video_res:  # 프레임 읽었는지 확인 -> 영상 실행중에 프레임을 읽어오는 데 문제가 생길 수도 있으니까..
            print("Video error")
            break  # 반복문 종료

    # 비디오 캡처 객체와 동영상 객체를 모두 인식하여 화면에 출력시킨다는 뜻.
