...
r = sr.Recognizer()  # Recognizer 객체 생성 => 음성 녹음 ㄱㄴ
# Recognizer() : 음성 인식 기능 모음을 나타내는 새 ``Recognizer`` 인스턴스를 만듭니다.
with sr.Microphone() as source:  # 마이크를 source로 사용
    print("녹음 시작")
    audio = r.record(source, duration=5)  # 5초 동안 마이크를 사용하여 녹음 => source 입력 : 마이크 사용 ㄱㄴ
    print("녹음 끝")
